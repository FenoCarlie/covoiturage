...
function ItineraryInfo() {
  ...
  const getItinerary = () => {
    setLoading(true);
    axiosClient
      .post("/search/courses", { id: itineraryId }) // Corrected to pass idData as the request body
      .then(({ data }) => {
        setItinerary(data[0]);
        const date = new Date(itinerary.dateDep.date).toLocaleDateString(
          "en-US",
          {
            month: "long",
            day: "numeric",
        ...
        {loading ? (
          ...
          <>
            ...
            <section className="w-[70%] flex justify-center h-full p-4">
              <h1>{import { useStateContext } from "../context/ContextProvider";
import "react-datepicker/dist/react-datepicker.css";
import axiosClient from "../axios-client";
import { useEffect, useState } from "react";
import { CiMail, CiPhone } from "react-icons/ci";
