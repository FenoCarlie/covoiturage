...
async function Insert(request, response) {
  ...
  if (field == "reservations") {
    ...
    reservation = await Find(field, {
      idCourses: stack.idCourses,
      idUsers: stack.idUsers,
    });
    if (reservation != null) {
      sweep.params.table = field;
      sweep.body = { id: reservation._id };
      reservation.seats += stack.seats;
      delete reservation._id;
      sweep.body.data = reservation;
      Update(sweep);
      response.send(reservation._id);
      return;
    }
...
function SearchIti(request, response) {
  var found, hay, stack, floor, raw, needle, room;
  raw = request.body;
  if (request == null || response == null || raw == undefined) {
    return;
  }
  ...
  for (needle in raw) {
    if (needle == "seats") {
      hay[needle] = { $not: { $lt: raw[needle] } };
      continue;
    }
    hay[needle] = { $regex: raw[needle] };
    hay[needle]["$options"] = "i";
  }
  request.body = hay;
  Select(request, response);
}
