...
function Stepper() {
  ...
  useEffect(() => {
    // Use the `useState` update function to update the `course` state
    setCourse((prevCourse) => ({ ...prevCourse, car_place: place }));

    // Use a more modern way to get the active element
    const active = document.querySelector(`#step-${activeStep}`);

    // Use a more modern way to get the input elements
    const inputs = active.querySelectorAll("input");

    // Use a more functional way to check if any input is empty
    const isEmpty = Array.from(inputs).some(
