...
const Course = () => {
  ...
  console.log(import.meta.env.OPENCAGE_GEOCODING_API_KEY_TOKEN);
    ...
    try {
      const responseStart = await axios.get(
        `https://api.opencagedata.com/geocode/v1/json?q=${
          course.loc_start
        }&key=${import.meta.env.OPENCAGE_GEOCODING_API_KEY_TOKEN}
        }`
      );
      setLocationStart(responseStart.data.results[0].geometry);
      console.log(responseStart);
      const responseEnd = await axios.get(
        `https://api.opencagedata.com/geocode/v1/json?q=${course.loc_end}&key=44528d1f7adc48259362a60e9f4f0054
        }`
      );
