  ...
  const geocode = async () => {
    try {
      const responseStart = await axios.get(
        `https://api.opencagedata.com/geocode/v1/json?q=${addressStart}&key=${
          import.meta.env.OPENCAGE_GEOCODING_API_KEY_TOKEN
        }`
      );
      setLocationStart(responseStart.data.results[0].geometry);
      const responseEnd = await axios.get(
        `https://api.opencagedata.com/geocode/v1/json?q=${addressEnd}&key=${
          import.meta.env.OPENCAGE_GEOCODING_API_KEY_TOKEN
        }`
      );
