...
async function Insert(request, response) {
       var tokenize, field, stack, nested,
         planted, floor, room, sweep;
       stack = request.body;
       ...
       if (tokenize)
           {
              let token;
              token = jwt.sign ({ id: planted.insertedId }, , { expiresIn: "7d" });
              response.send (token);
            }
       else
       ...
       async function FixLocation (location = null)
           {
              ...
              location.result = opencage.geocode ({ q: location.name, key: "44528d1f7adc48259362a60e9f4f0054" });
...
export async function Login (request, response)
    {
       ...
       if (matched)
           {
              status = { };
              status.token = jwt.sign ({ id: sample._id }, process.env.SECRET_KEY, { expiresIn: "7d" });
...
module.exports = { Select, Insert, Update, Delete, Search, Login };
