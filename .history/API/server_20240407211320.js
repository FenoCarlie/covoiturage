...
app.use(async (req, res, next) => {
  ...
  try {
    ...
    // Vérifier si le token a expiré ou non
    if (jwt.decode(token, process.env.SECRET_KEY).exp < Date.now() / 1000) {
      // Générer un nouveau token
      const newToken = jwt.sign({ id: req.user._id }, process.env.SECRET_KEY, {
        expiresIn: "7d",
      });

      // Ajouter le nouveau token au header de la réponse
