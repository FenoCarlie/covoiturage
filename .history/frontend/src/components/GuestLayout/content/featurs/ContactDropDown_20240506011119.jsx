...
export default function ContactDropDown() {
  var res = phoneCode.filter(function (item) {
    return item.name == "Madagascar";
  });

  const [state, setState] = useState(res[0].name);
  const [dial_code, setDial_code] = useState(res[0].dial_code);
  const [flag, setFlag] = useState(res[0].image);
  const [code, setCode] = useState(res[0].code);

  var setPhoneCode = (state, dial_code, flag, code) => {
    setState(state);
