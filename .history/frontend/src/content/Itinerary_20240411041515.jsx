...
function Itinerary({ searchData }) {
  ...
  const searchItinerary = (data) => {
    const payload = {
      locStart: data.from,
      locEnd: data.to,
      //dateDep: data.date,
      place: data.passengers,
    };
    console.log(payload);

    const filteredData = itinerary.filter(
      (item) =>
        item.locStart === search.from &&
        item.locEnd === search.to &&
        //item.dateDep === search.date &&
        item.place === search.passengers
    );

    console.log(filteredData);
    return setItinerary(filteredData);
  };
