...
const Course = () => {
  ...
  const geocode = async () => {
    setAddressStart(course.loc_start);
    setAddressEnd(course.loc_end);
    console.log("in geocode");
    try {
      const responseStart = await axios.get(
        `https://api.opencagedata.com/geocode/v1/json?q=${addressStart}&key=b44772c85f8048ccb87cc4c7805b7a16
        https://api.opencagedata.com/geocode/v1/json?q=Paris&key=b44772c85f8048ccb87cc4c7805b7a16
        }`
      );
      setLocationStart(responseStart.data.results[0].geometry);
      const responseEnd = await axios.get(
        `https://api.opencagedata.com/geocode/v1/json?q=${addressEnd}&key=b44772c85f8048ccb87cc4c7805b7a16
        }`
      );
      setLocationEnd(responseEnd.data.results[0].geometry);
    } catch (error) {
